...
def main(_argv):
    ...
    polygons=[]
    number_of_polygon=int(len(points)/4)
    ...
    while True:
        ...
        #match cars with lanes
            for id_polygon,polygon in enumerate(polygons): 
                if polygon.contains(Point((bbox[0]+bbox[2])/2,bbox[3])):
                    if (track.track_id in lanes_matched_ids[id_polygon])==False:
                        print("Tracker ID: {}, Class: {}".format(str(track.track_id), class_name)) 
                        print(id_polygon)
                        lanes_matched_ids[id_polygon].append(track.track_id)
                        number_of_vehicles[id_polygon][class_name_dict[class_name]]+=1
